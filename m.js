...
/* ＋ 새 곳 등록 — 지도 링크를 붙이면 서버가 이름·전화·영업시간·메뉴를 채운다 */
function sheetRestNew() {
  const p = RS_ADD;
  return `<div class="sh-title">＋ ${t("새 곳 등록", "Add a place")}</div>
    <div class="sh-sub">${t("네이버·구글 지도에서 '공유' 한 링크를 붙여 넣으세요",
      "Paste a share link from Naver or Google Maps")}</div>
    <div class="frm">
      <label>${t("지도 링크", "Map link")}<input id="rsUrl" type="url" inputmode="url"
        placeholder="https://naver.me/… " value="${esc((p && p.url) || "")}"></label>
      <label class="inline">${t("어디에", "Where")}
        <select id="rsKind">
      ...
      <label>${t("한 줄 추천 (선택)", "One-line tip (optional)")}<input id="rsNote"></label>
    </div>
    ${p && p.place && p.place.name
      ? `<div class="rs-reco">✅ <b>${esc(p.place.name)}</b><br>
          <span class="muted">${esc(p.place.category || "")} ${esc(p.place.road_address || p.place.address || "")}</span>
          ${p.duplicate ? `<br>⚠️ ${t("이미 올라온 곳이에요 — 등록하면 그 카드에 합쳐져요",
            "Already on the board — this will merge into that card")}` : ""}</div>` : ""}
    <button class="ics-btn" data-act="rslook">${t("링크 불러오기", "Fetch link")}</button>
    ${p && p.place && p.place.name
      ? `<button class="ics-btn" data-act="rsadd">${t("등록하기", "Add it")}</button>` : ""}
    <p class="pie-note center">${t("불러오는 데 5~15초 걸릴 수 있어요 (지도에서 긁어옵니다)",
      "Fetching can take 5–15s (we read the map page)")}</p>`;
}
...
document.addEventListener("click", (e) => {
  ...
  if (act === "gotab") {
    ...
    const u = ($("#rsUrl").value || "").trim();
    if (!u) { toast(t("지도 링크를 붙여 넣어 주세요", "Paste a map link")); return; }
    busyRun(b, async () => {
      const res = await rpc({ op: "rest_lookup", url: u });
      RS_ADD = { url: u, place: res.place || {}, duplicate: res.duplicate || null };
      openSheet(sheetRestNew());
      if (!RS_ADD.place.name) toast(t("이름을 못 읽었어요 — 다른 링크로 해보세요", "Couldn't read it — try another link"));
    });
  } else if (act === "rsadd") {
    const kind = $("#rsKind").value, note = ($("#rsNote").value || "").trim();
    busyRun(b, async () => {
      await rpc({ op: "rest_add", url: (RS_ADD && RS_ADD.url) || "", place: (RS_ADD && RS_ADD.place) || {},
                  kind, note });
      RS_KIND = kind; RS_ADD = null;
      afterWrite(t("등록했어요", "Added"));
      renderRest();
    });
  } else if (act === "rsvote") {
