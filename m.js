...
function renderAll() {
  applyStaticText();
  renderHeader(); renderCal(); renderMy(); renderServers(); renderRest();
  renderMembers(); renderApprovals(); renderGrants(); renderMore();
}
...
/* ─── 🍽️ 음식점 — 랩원이 올린 맛집 카드 ───
   연구실 /restaurants.html 과 같은 얼개: 처음엔 **🍽️ 음식점 / ☕ 카페 두 갈래**, 들어가면 목록.
   폰에서 궁금한 건 '지금 여나 · 얼마나 머나 · 뭐 먹나 · 랩원 평가'다. 👍👎·가봤어요·
   가보고 싶어요·코멘트는 **폰에서 바로 누른다** — 본인 KEK 로 확인해 연구실 게이트웨이에
   전달하고(RPC), 누른 결과는 다음 발행(≤3분)에 다른 폰에도 보인다. */
const RS_ICO = { food: "🍽️", cafe: "☕" };
const RS_NAME = (k) => (k === "cafe" ? t("카페", "Cafes") : t("음식점", "Restaurants"));
let RS_KIND = "";          // "" = 두 갈래 첫 화면 · food · cafe
let RS_SORT = "like";      // like | dist | rate
let RS_Q = "";             // 검색어
let RS_MINE = false;       // 💛·✅ 내가 표시한 것만
let RS_OPEN = false;       // 지금 영업 중만
let RS_ADD = null;         // ＋ 등록 중인 카드 미리보기

const restAll = () => (P && P.rest) || [];
const restOf = (id) => restAll().find((r) => r.i === id);
const hm2min = (s) => { const [h, m] = String(s || "").split(":").map(Number); return (h || 0) * 60 + (m || 0); };

/* GIST 본원에서 직선거리 — 연구실 화면과 같은 기준점(실제 길은 1.2~1.4배) */
const GIST = { lat: 35.2289, lng: 126.8431 };
function restKm(xy) {
  if (!xy) return null;
  const R = 6371, toR = (d) => (d * Math.PI) / 180;
  const dLat = toR(xy[0] - GIST.lat), dLng = toR(xy[1] - GIST.lng);
  const h = Math.sin(dLat / 2) ** 2 + Math.cos(toR(GIST.lat)) * Math.cos(toR(xy[0])) * Math.sin(dLng / 2) ** 2;
  return 2 * R * Math.asin(Math.sqrt(h));
}
const restKmTxt = (km) => (km == null ? "" : km < 1 ? `${Math.round(km * 1000 / 10) * 10}m` : `${km.toFixed(1)}km`);

/* 지금 상태 — 서버가 긁은 문구는 금방 낡으니 화면이 현재 시각으로 판정한다 */
function restNow(r) {
  const s = (r.h || [])[(new Date().getDay() + 6) % 7] || "";
  if (!s) return null;
  if (s === "휴무") return { k: "off", t: t("오늘 휴무", "Closed today") };
  const [span, ...brs] = s.split("/");
  const [o, c] = span.split("-");
  if (!o || !c) return null;
  const now = new Date().getHours() * 60 + new Date().getMinutes();
  const open = hm2min(o);
  let close = hm2min(c);
  if (close <= open) close += 1440;                        // 새벽까지 하는 집
  const cur = (now < open && close > 1440) ? now + 1440 : now;
  if (cur < open) return { k: "pre", t: t(`${o} 영업 시작`, `opens ${o}`) };
  if (cur >= close) return { k: "end", t: t("영업 종료", "Closed now") };
  for (const b of brs) {
    const [bs, be] = b.split("-");
    if (now >= hm2min(bs) && now < hm2min(be)) return { k: "brk", t: t(`브레이크 ~${be}`, `break till ${be}`) };
  }
  return { k: "on", t: t(`${c}까지`, `till ${c}`) };
}

const restMyVote = (r) => ((r.vt || {})[ME && ME.name] || 0);
const restMine = (r) => (r.vs || []).includes(ME && ME.name) || (r.ws || []).includes(ME && ME.name);

function restCard(r) {
  const st = restNow(r);
  const km = restKm(r.xy);
  const faces = (r.vs || []).slice(0, 3).map((n) => facePh(n, "rs-face")).join("");
  const mine = restMyVote(r);
  return `<button class="card rs" data-sheet="rest" data-id="${esc(r.i)}">
    ${r.ph ? `<img class="rs-th" src="${esc(r.ph)}" alt="" loading="lazy">`
           : `<span class="rs-th none">${RS_ICO[r.k] || "🍽️"}</span>`}
    <span class="rs-b">
      <span class="rs-n">${esc(r.n)}${mine ? ` <i class="rs-my">${mine > 0 ? "👍" : "👎"}</i>` : ""}</span>
      <span class="rs-sub">${esc(r.c)}${r.ra ? ` · ★ ${r.ra}` : ""}${km != null ? ` · 📏 ${restKmTxt(km)}` : ""}</span>
      <span class="rs-chips">
        ${st ? `<span class="rs-st ${st.k}">${esc(st.t)}</span>` : ""}
        ${r.lk ? `<span class="rs-v">👍 ${r.lk}</span>` : ""}${r.dl ? `<span class="rs-v">👎 ${r.dl}</span>` : ""}
        ${r.cm ? `<span class="rs-v">💬 ${r.cm}</span>` : ""}
        ${faces ? `<span class="rs-faces">${faces}${(r.vs || []).length > 3 ? `<i>+${r.vs.length - 3}</i>` : ""}</span>` : ""}
      </span>
      ${r.no ? `<span class="rs-note">${esc(r.no)}</span>` : ""}
    </span></button>`;
}

/* 첫 화면 — 🍽️ 음식점 / ☕ 카페 (연구실 화면과 같은 두 갈래, PI 지시 2026-09-18) */
function restLanding() {
  const card = (k) => {
    const rows = restAll().filter((r) => r.k === k);
    const open = rows.filter((r) => (restNow(r) || {}).k === "on").length;
    return `<button class="rs-lcard" data-act="rskind" data-k="${k}">
      <span class="rs-lico">${RS_ICO[k]}</span>
      <span class="rs-lb"><b>${RS_NAME(k)}</b>
        <span class="muted">${t(`${rows.length}곳 · 지금 영업 ${open}`, `${rows.length} places · ${open} open now`)}</span></span>
      <span class="row-sub">›</span></button>`;
  };
  return `<div class="my-head card"><div><b>🍽️ ${t("음식점", "Eats")}</b><br>
      <span class="muted" style="font-size:12px">${t("랩원이 올린 맛집 — 들어갈 곳을 고르세요",
        "Places the lab collected — pick where to go in")}</span></div></div>
    <div class="rs-land">${card("food")}${card("cafe")}</div>
    <button class="ics-btn" data-act="rsnew">＋ ${t("새 곳 등록", "Add a place")}</button>
    <p class="more-meta">${t("지도 링크만 붙이면 전화·영업시간·메뉴가 저절로 채워져요",
      "Paste a map link — phone, hours and menu fill in by themselves")}</p>`;
}
...
function renderRest() {
  ...
  if (!RS_KIND) { box.innerHTML = restLanding(); return; }

  const me = ME && ME.name;
  let list = rows.filter((r) => r.k === RS_KIND);
  if (RS_OPEN) list = list.filter((r) => (restNow(r) || {}).k === "on");
  if (RS_MINE) list = list.filter(restMine);
  if (RS_Q) {
    const q = RS_Q.toLowerCase();
    list = list.filter((r) => [r.n, r.c, r.a, r.no, ...(r.tg || [])]
      .some((x) => String(x || "").toLowerCase().includes(q)));
  }
  if (RS_SORT === "dist") list = [...list].sort((a, b) => (restKm(a.xy) ?? 1e9) - (restKm(b.xy) ?? 1e9));
  else if (RS_SORT === "rate") list = [...list].sort((a, b) => (b.ra || 0) - (a.ra || 0));
  const nOpen = rows.filter((r) => r.k === RS_KIND && (restNow(r) || {}).k === "on").length;

  let h = `<div class="my-head card">
      <button class="chip-btn" data-act="rskind" data-k="">‹ ${t("뒤로", "Back")}</button>
      <div><b>${RS_ICO[RS_KIND]} ${RS_NAME(RS_KIND)}</b><br>
      <span class="muted" style="font-size:12px">${t(`${list.length}곳 · 지금 영업 ${nOpen}`,
        `${list.length} places · ${nOpen} open`)}</span></div>
      <button class="chip-btn add" data-act="rsnew">＋</button></div>
    <input class="rs-q" id="rsQ" type="search" value="${esc(RS_Q)}"
      placeholder="${t("이름·메뉴·태그 검색", "Search name, tag…")}">
    <div class="rs-tabs">
      <button class="rs-tab${RS_OPEN ? " on" : ""}" data-act="rsflt" data-k="open">🟢 ${t("지금 영업", "Open now")}</button>
      <button class="rs-tab${RS_MINE ? " on" : ""}" data-act="rsflt" data-k="mine">💛 ${t("내 목록", "Mine")}</button>
      <button class="rs-tab${RS_SORT === "like" ? " on" : ""}" data-act="rssort" data-k="like">👍 ${t("추천순", "Top")}</button>
      <button class="rs-tab${RS_SORT === "dist" ? " on" : ""}" data-act="rssort" data-k="dist">📏 ${t("가까운 순", "Nearest")}</button>
      <button class="rs-tab${RS_SORT === "rate" ? " on" : ""}" data-act="rssort" data-k="rate">★ ${t("별점순", "Rating")}</button>
    </div>`;
  h += list.length ? list.map(restCard).join("")
    : `<div class="empty">${t("조건에 맞는 곳이 없어요", "Nothing matches")}</div>`;
  h += `<p class="more-meta">${t("📏 는 GIST에서 직선거리 · 실제 길은 조금 더 멀어요",
    "📏 straight-line from GIST — the real route is a bit longer")}</p>`;
  box.innerHTML = h;
  const q = $("#rsQ");
  if (q) {
    q.oninput = () => { RS_Q = q.value.trim(); clearTimeout(q._tm); q._tm = setTimeout(() => {
      const at = q.selectionStart; renderRest();
      const nq = $("#rsQ"); if (nq) { nq.focus(); nq.setSelectionRange(at, at); }
    }, 220); };
  }
}

/* 상세 — 여기서 누르는 것들이 그대로 연구실 대시보드에 반영된다 */
function sheetRest(id) {
  const r = restOf(id);
  if (!r) return `<div class="sh-title">🍽️</div>`;
  const me = ME && ME.name;
  const st = restNow(r);
  const km = restKm(r.xy);
  const mine = restMyVote(r);
  const been = (r.vs || []).includes(me), wish = (r.ws || []).includes(me);
  const DOW = LANG === "en" ? ["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"]
                            : ["월", "화", "수", "목", "금", "토", "일"];
  const today = (new Date().getDay() + 6) % 7;
  const hours = (r.h || []).map((s, i) => `<div class="rs-hr${i === today ? " now" : ""}">
    <span>${DOW[i]}</span><b>${esc(s === "휴무" ? t("휴무", "Closed") : (s || "—").replace("/", " · "))}</b></div>`).join("");
  const people = (label, names) => (names || []).length
    ? `<div class="rs-ppl"><span class="muted">${label}</span>${names.map((n) =>
        `<span class="rs-p">${facePh(n, "rs-face")}<i>${esc(personLabel(n))}</i></span>`).join("")}</div>` : "";
  const cmts = (r.cs || []).map((c, i) => `<div class="rs-cm">
      <b>${esc(personLabel(c.w))}</b> ${esc(c.t)}
      ${c.w === me && c.id ? `<button class="rs-del" data-act="rsuncm" data-id="${esc(r.i)}" data-cid="${esc(c.id)}">🗑</button>` : ""}
    </div>`).join("");
  return `<div class="sh-title">${RS_ICO[r.k] || "🍽️"} ${esc(r.n)}</div>
    <div class="sh-sub">${esc(r.c)}${r.ra ? ` · ★ ${r.ra} (${r.rv || 0})` : ""}
      ${km != null ? ` · 📏 ${restKmTxt(km)}` : ""}${st ? ` · <b class="rs-st ${st.k}">${esc(st.t)}</b>` : ""}</div>
    ${r.ph ? `<img class="rs-hero" src="${esc(r.ph)}" alt="" loading="lazy">` : ""}
    <div class="rs-acts">
      ${r.p ? `<a class="wr-btn" href="tel:${esc(String(r.p).replace(/[^0-9+]/g, ""))}">📞 ${t("전화", "Call")}</a>` : ""}
      ${r.u ? `<a class="wr-btn" href="${esc(r.u)}" target="_blank" rel="noopener">🗺️ ${t("지도", "Map")}</a>` : ""}
    </div>
    <div class="rs-btns">
      <button class="rs-bt${mine > 0 ? " on" : ""}" data-act="rsvote" data-id="${esc(r.i)}" data-v="1">👍 <b>${r.lk || 0}</b></button>
      <button class="rs-bt${mine < 0 ? " on" : ""}" data-act="rsvote" data-id="${esc(r.i)}" data-v="-1">👎 <b>${r.dl || 0}</b></button>
      <button class="rs-bt${been ? " on" : ""}" data-act="rsmark" data-id="${esc(r.i)}" data-kind="visited">${been ? "✅" : "☑️"} ${t("가봤어요", "Been")} <b>${(r.vs || []).length}</b></button>
      <button class="rs-bt${wish ? " on" : ""}" data-act="rsmark" data-id="${esc(r.i)}" data-kind="wish">💛 ${t("가보고 싶어요", "Want")} <b>${(r.ws || []).length}</b></button>
    </div>
    ${r.a ? `<div class="rs-addr">📍 ${esc(r.a)}</div>` : ""}
    ${r.no ? `<div class="rs-reco">💬 ${esc(r.no)}</div>` : ""}
    ${(r.tg || []).length ? `<div class="allday">${r.tg.map((x) =>
      `<span class="ad-chip"><span class="t">#${esc(x)}</span></span>`).join("")}</div>` : ""}
    <div class="sh-sec">${t("영업시간", "Hours")}${r.cn ? ` · ${esc(r.cn)}` : ""}</div>
    <div class="rs-hrs">${hours}</div>
    ${(r.mn || []).length ? `<div class="sh-sec">${t("메뉴", "Menu")}</div>
      <div class="rs-menu">${r.mn.map((m) => `<div class="rs-mi">
        <span>${m.r ? "★ " : ""}${esc(m.n)}</span><b>${esc(m.p ? Number(m.p).toLocaleString() : "")}</b></div>`).join("")}</div>` : ""}
    ${people(t("다녀왔어요", "Been there"), r.vs)}
    ${people(t("가보고 싶어요", "Want to go"), r.ws)}
    <div class="sh-sec">${t("코멘트", "Comments")}</div>
    ${cmts || `<div class="empty small">${t("첫 코멘트를 남겨보세요", "Be the first to comment")}</div>`}
    <div class="frm"><label>${t("한 줄 남기기", "Leave a line")}
      <textarea id="rsCm" rows="2" placeholder="${t("맛·웨이팅·주차 같은 걸 적어주세요", "Taste, waiting, parking…")}"></textarea></label></div>
    <button class="ics-btn" data-act="rscm" data-id="${esc(r.i)}">💬 ${t("코멘트 남기기", "Post comment")}</button>`;
}

/* ＋ 새 곳 등록 — 링크를 붙이면 서버가 채우고, 못 읽으면 **이름만 적어도** 등록된다.
   (PI 제보 2026-09-21: 카카오맵 링크·'공유 문구째 붙여넣기'에서 막혀 아무것도 못 하고 끝났다) */
function sheetRestNew() {
  const p = RS_ADD || {};
  const pl = p.place || {};
  const kindSel = `<label class="inline">${t("어디에", "Where")}
      <select id="rsKind">
        <option value="food"${RS_KIND !== "cafe" ? " selected" : ""}>🍽️ ${t("음식점", "Restaurants")}</option>
        <option value="cafe"${RS_KIND === "cafe" ? " selected" : ""}>☕ ${t("카페", "Cafes")}</option>
      </select></label>`;
  return `<div class="sh-title">＋ ${t("새 곳 등록", "Add a place")}</div>
    <div class="sh-sub">${t("네이버·구글 지도에서 '공유'한 링크를 붙여 넣으세요 — 앞뒤에 글자가 섞여 있어도 괜찮아요",
      "Paste a share link from Naver or Google Maps — surrounding text is fine")}</div>
    <div class="frm">
      <label>${t("지도 링크", "Map link")}<input id="rsUrl" type="text" inputmode="url"
        autocapitalize="off" spellcheck="false" placeholder="https://naver.me/… "
        value="${esc(p.url || "")}"></label>
    </div>
    <button class="ics-btn" data-act="rslook">🔎 ${t("링크 불러오기", "Fetch link")}</button>
    ${p.err ? `<div class="rs-warn">⚠️ ${esc(p.err)}<br>
        ${t("링크가 안 읽히면 아래에 <b>이름만</b> 적어도 등록돼요.",
            "If the link won't load, just type the <b>name</b> below.")}</div>` : ""}
    ${pl.name ? `<div class="rs-reco">✅ <b>${esc(pl.name)}</b>
        <span class="muted">${esc(pl.category || "")} ${esc(pl.road_address || pl.address || "")}</span>
        ${p.duplicate ? `<br>⚠️ ${t("이미 올라온 곳이에요 — 등록하면 그 카드에 합쳐져요(내 👍·한 줄이 같이 들어가요)",
          "Already on the board — this merges into that card")}` : ""}</div>` : ""}
    <div class="frm">
      <label>${t("이름", "Name")}<input id="rsName" value="${esc(pl.name || "")}"
        placeholder="${t("예: 카페 더하루", "e.g. Cafe Haru")}"></label>
      <label>${t("분류 (선택)", "Category (optional)")}<input id="rsCat" value="${esc(pl.category || "")}"
        placeholder="${t("카페·디저트 같은 것", "cafe, dessert…")}"></label>
      <label>${t("전화 (선택)", "Phone (optional)")}<input id="rsTel" type="tel" value="${esc(pl.phone || "")}"></label>
      ${kindSel}
      <label>${t("한 줄 추천 (선택)", "One-line tip (optional)")}<input id="rsNote" value="${esc(p.note || "")}"></label>
    </div>
    <button class="ics-btn" data-act="rsadd">${t("등록하기", "Add it")}</button>
    <p class="pie-note center">${t("불러오는 데 5~15초 걸릴 수 있어요 · 등록하면 연구실 대시보드에도 바로 보여요",
      "Fetching can take 5–15s · it shows up on the lab dashboard too")}</p>`;
}
...
document.addEventListener("click", (e) => {
  ...
  if (act === "gotab") {
  ...
  } else if (act === "rskind") {          /* 🍽️ 두 갈래 첫 화면 ↔ 목록 */
    RS_KIND = b.dataset.k || ""; RS_Q = ""; RS_OPEN = false; RS_MINE = false;
    renderRest(); $("#main").scrollTop = 0;
  } else if (act === "rsflt") {
    if (b.dataset.k === "open") RS_OPEN = !RS_OPEN; else RS_MINE = !RS_MINE;
    renderRest();
  } else if (act === "rssort") {
    RS_SORT = b.dataset.k; renderRest();
  } else if (act === "rsnew") {
    if (!rpcUrl()) { toast(t("연구실 통로가 아직 연결 안 됐어요 — 잠시 후 다시", "Not connected yet — try again")); return; }
    RS_ADD = null; openSheet(sheetRestNew());
  } else if (act === "rslook") {
    const raw = ($("#rsUrl").value || "").trim();
    const u = (raw.match(/https?:\/\/\S+/i) || [raw])[0];     /* 공유 문구째 붙여넣어도 링크만 */
    if (!u) { toast(t("지도 링크를 붙여 넣어 주세요", "Paste a map link")); return; }
    const note = ($("#rsNote") && $("#rsNote").value.trim()) || "";
    busyRun(b, async () => {
      try {
        const res = await rpc({ op: "rest_lookup", url: u });
        RS_ADD = { url: u, place: res.place || {}, duplicate: res.duplicate || null, note,
                   err: (res.place && res.place.name) ? "" : t("이 링크에서는 가게 이름을 못 읽었어요",
                                                               "Couldn't read a place name from this link") };
      } catch (e) {
        RS_ADD = { url: u, place: {}, duplicate: null, note, err: e.message };
      }
      openSheet(sheetRestNew());
    });
  } else if (act === "rsadd") {
    const name = ($("#rsName").value || "").trim();
    if (!name) { toast(t("이름을 적어주세요", "Type a name")); return; }
    const body = { op: "rest_add", url: (RS_ADD && RS_ADD.url) || "", place: (RS_ADD && RS_ADD.place) || {},
      kind: $("#rsKind").value, note: ($("#rsNote").value || "").trim(),
      name, category: ($("#rsCat").value || "").trim(), phone: ($("#rsTel").value || "").trim() };
    busyRun(b, async () => {
      const res = await rpc(body);
      RS_KIND = body.kind; RS_ADD = null;
      closeSheet();
      holdUntil = Date.now() + 150000;
      toast(res && res.merged ? t("이미 있던 카드에 합쳤어요", "Merged into the existing card")
                              : t("등록했어요 · 1~2분 뒤 목록에 보여요", "Added · shows up in 1–2 min"));
    });
  } else if (act === "rsvote") {
    const r = restOf(b.dataset.id), me = ME && ME.name;
    if (!r) return;
    const want = Number(b.dataset.v), cur = restMyVote(r);
    const value = cur === want ? 0 : want;               /* 같은 걸 다시 누르면 취소 */
    busyRun(b, async () => {
      await rpc({ op: "rest_vote", id: r.i, value });
      r.vt = r.vt || {};
      if (value) r.vt[me] = value; else delete r.vt[me];
      r.lk = Object.values(r.vt).filter((v) => v === 1).length;
      r.dl = Object.values(r.vt).filter((v) => v === -1).length;
      openSheet(sheetRest(r.i)); renderRest();
      holdUntil = Date.now() + 150000;
      toast(value === 1 ? "👍" : value === -1 ? "👎" : t("취소했어요", "Undone"));
    });
  } else if (act === "rsmark") {
    const r = restOf(b.dataset.id), me = ME && ME.name, kind = b.dataset.kind;
    if (!r) return;
    const box = kind === "wish" ? (r.ws = r.ws || []) : (r.vs = r.vs || []);
    const on = !box.includes(me);
    busyRun(b, async () => {
      await rpc({ op: "rest_mark", id: r.i, kind, value: on });
      if (on) {
        box.push(me);
        if (kind === "visited") r.ws = (r.ws || []).filter((n) => n !== me);   /* 가봤으면 위시는 이룬 것 */
      } else {
        const i = box.indexOf(me); if (i >= 0) box.splice(i, 1);
      }
      openSheet(sheetRest(r.i)); renderRest();
      holdUntil = Date.now() + 150000;
      toast(on ? (kind === "wish" ? "💛" : "✅") : t("취소했어요", "Undone"));
    });
  } else if (act === "rscm") {
    const r = restOf(b.dataset.id), me = ME && ME.name;
    const el = $("#rsCm"), text = ((el && el.value) || "").trim();
    if (!r || !text) { toast(t("한 줄 적어주세요", "Write a line first")); return; }
    busyRun(b, async () => {
      await rpc({ op: "rest_comment", id: r.i, text });
      r.cs = [...(r.cs || []), { w: me, t: text }].slice(-3);
      r.cm = (r.cm || 0) + 1;
      openSheet(sheetRest(r.i)); renderRest();
      holdUntil = Date.now() + 150000;
      toast(t("남겼어요", "Posted"));
    ...
    const r = restOf(b.dataset.id);
    if (!r) return;
    busyRun(b, async () => {
      await rpc({ op: "rest_uncomment", id: r.i, cid: b.dataset.cid });
      r.cs = (r.cs || []).filter((c) => c.id !== b.dataset.cid);
      r.cm = Math.max(0, (r.cm || 1) - 1);
      openSheet(sheetRest(r.i)); renderRest();
      holdUntil = Date.now() + 150000;
      toast(t("지웠어요", "Deleted"));
    });
  } else if (act === "srvnow") {
...
document.addEventListener("click", (e) => {
  ...
  switch (el.dataset.sheet) {
    ...
    case "rest": openSheet(sheetRest(id)); break;
...
document.querySelectorAll("#tabbar .tab").forEach((b) => b.addEventListener("click", () => {
  ...
  ["cal", "my", "srv", "rs", "mem", "ap", "gr", "more"].forEach((v) => { $("#view-" + v).hidden = v !== b.dataset.view; });
...
function applyStaticText() {
  ...
  const tabs = [t("캘린더", "Calendar"), t("내 공간", "My space"), t("서버", "Servers"),
                t("음식점", "Eats"), t("멤버", "Members"), t("승인", "Approve"),
                t("과제찾기", "Grants"), t("더보기", "More")];
